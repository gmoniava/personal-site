...
- Static Generation with `generateStaticParams`: This function ensures
that each blog post is statically generated at build time.

- Metadata Generation with `generateMetadata: This function is used to
dynamically generate metadata (such as title, description, etc.) for each blog post. Since the blog
pages are statically generated, the metadata is also generated at build
time.

- Structured Data with JSON-LD: The page includes structured data in the
...
- Sitemap and RSS: The starter kit also had a sitemap (`app/sitemap.ts`), a file which has information to help search engines crawl the site,
...
- Custom MDX Component: I had to add custom component for `pre` tag in the `components` object (`app/components/server/mdx.tsx`).
...
- Code Syntax Highlighting Theme Fix: The starter code also uses a code syntax highlighter ([`sugar-high`](https://github.com/huozhi/sugar-high)) to highlight code blocks.
