...
The way this blog works is there is a server component (`app/components/server/posts.tsx`) which retrieves posts (which are [MDX](https://mdxjs.com/) files) from
...
(`app/blog/[slug]/page.tsx`). The client component also allows filtering of the posts.
...
- you have to add [this](https://tailwindcss.com/docs/dark-mode#toggling-dark-mode-manually) line: `@custom-variant dark (&:where(.dark, .dark *));`,
  to the `global.css`. It will enable `dark:` utilities to be applied when dark class is present in HTML tree, instead of
...
Finally, you will in that page usage of JSON-LD which is used to provide structured data that improves search engine understanding.
