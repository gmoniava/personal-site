...
The main structure of the blog is that you have a server component (`app/components/server/posts.tsx`) which retrieves
the posts from the filesystem and passes them as props to a client component (`app/components/client/posts.tsx`).
The client component generates links for each post, which the user can click to navigate to the individual post page
located at `app/blog/[slug]/page.tsx`.
...
to the new URL, since changes won’t be visible immediately. The `useOptimistic` hook helps mitigate such issues.
This [post](https://buildui.com/posts/instant-search-params-with-react-server-components)
offers a great exploration of how to use `useOptimistic` to build URL-driven filter panels.
...
function invocations, I decided to add `export const dynamic = "force-static"` to both files as
described [here](https://nextjs.org/docs/app/api-reference/file-conventions/route-segment-config)
