---
title: "How I Built This Blog Using Nextjs"
...
There were some changes I made and functionality I added (filtering of the posts and theme switcher) which I will tell you about in this post.
...
update the filter values using a select component, which would
...
panel to support URL parameters. As a first step to achieve this, I decided to use the native History API (`history.pushState`) to update the URL
...
system theme. But you did not have a theme switcher component. There were certain things I had to do to make the manual theme switching work:
...
## Changes for MDX and Code Syntax Highlighter

There were also few changes related to MDX and code syntax highlighter ([`sugar-high`](https://github.com/huozhi/sugar-high)) used by the starter kit.

I had to add custom component for `pre` tag also in the `components` object (`app/components/server/mdx.tsx`),
otherwise it was not differentiating between inline and block level code.

The starter code had also some overrides in the `global.css` for some tokens of the code syntax highlighter for the dark theme,
