...
The starter kit also had a `app/sitemap.ts` file (which is basically a
special Route Handler) and route handler (`app/rss/route.ts`) for
generating RSS. Both were relying on usage of the `getBlogPosts` method mentioned above,
which could potentially increase the number of server function invocations depending how
often it would be called. To stay on the safe side, I added below line in both files:
