...
in psychology which seems to be very related to it,
it is called _core beliefs_, from cognitive behavioral therapy (CBT).
I do not plan to explore this concept from CBT perspective (though there is likely overlap),
if you’re interested in that concept, you can look it up for more information.
I also discuss why changing beliefs might be difficult.
...
When we evaluate new information and form opinions, we can do so with two different goals in mind: either to discover the truth or to confirm our desired conclusions. When the latter happens, it's called _motivated reasoning_. It’s worth noting that this process often occurs unconsciously.

Let’s look at an example:

If you plan to go to a concert on Tuesday and develop a runny nose on Monday, you might tell yourself it’s just allergies or the weather — anything but the start of a cold or the flu. You don’t want to face the possibility that you might be getting sick and have to cancel, so you look for alternative explanations.

This example also highlights an important feature of motivated reasoning: it often has limits. You might convince yourself the runny nose is due to allergies — up to a point. But once you also develop a fever, it becomes much harder to ignore the more likely explanation.
