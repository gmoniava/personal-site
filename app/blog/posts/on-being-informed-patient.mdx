...
There is a test in cardiology called a stress test, where the patient
walks on a treadmill to make the heart work harder, while an
electrocardiogram (ECG) monitors the heart’s electrical activity. The
goal is to look for signs of coronary artery disease (CAD). The idea is
that when the heart works harder, a narrowed coronary artery caused by
CAD may not be able to supply enough blood to meet the heart’s increased
demand and this can be detected by the test. This test is often
recommended when a patient has chest pain that may be related to the
heart.
...
One of the reasons likely is the following. As noted stress testing can be helpful to detect narrowing in one or more coronary arteries.
However, it detects those cases, when there is 70% or more narrowing. Heart attacks on the other hand often happen when smaller narrowings (less than 70%)
