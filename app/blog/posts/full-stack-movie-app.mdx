...
- React uses the same component tree (that was used during SSR) on the
  client and renders it in memory and then checks that this output matches the HTML sent from the server.
- React will attach event listeners to the HTML sent by the server to make it interactive.
