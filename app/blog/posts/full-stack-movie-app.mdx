...
summary: "In this post, I share the project structure of a full-stack Next.js web app with explanations of folders and files and some issues I encountered and how I solved them."
...
### How I Dealt With Local Storage

In my app I am storing the state of the side bar (whether it is opened or not) in local storage.
Initially I decided to use `useLocalStorage` hook from `@uidotdev/usehooks` package. But it was giving me this error:

> Error: useLocalStorage is a client-only hook

despite using it in a component which had `"use client"`.
There is a [solution](https://github.com/uidotdev/usehooks/issues/218#issuecomment-1835624086) to this which looks like this:
...
But in my case the component that was using the hook was a layout, so if used my layout as shown above (instead of `MyComponentThatUsesHooks`),
it would mean the whole app beneath my layout would skip server side rendering (SSR) - due to the way `ClientOnly` hook works.
So I ended up using a different [implementation](https://github.com/gmoniava/movie-app/blob/main/src/hooks/useLocalStorage.ts).
