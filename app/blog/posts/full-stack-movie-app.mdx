...
## Fixing Hydration Errors and Initial Render Flicker

Sometimes in Next.js you may encounter hydration errors. Before we
...
A hydration error happens when the HTML rendered on the server doesn’t
match what React renders on the client in memory during hydration.

One cause of this error is if your render logic depends on local
storage. During SSR local storage is not available, hence the render
result you get during SSR and on client, may differ.

### Basic Client-Only Fix

A simple solution is to skip SSR for the component entirely:
...
I store the sidebar state in local storage and need it in some
components such as the
[layout](https://github.com/gmoniava/movie-app/blob/main/src/app/(main)/layout.tsx).
If I tried reading this value directly in the `useState` initializer
function in my layout, it would cause hydration error. I would also not
be able to fix this using above client-only fix because it would opt out
children of layout from SSR, which is my whole app. Besides I am not
sure if reading local storage in `useState` initializer function is ok
since it has to be pure.
...
function. If you read this value from `useEffect` it will not result in
hydration error because `useEffect` runs after hydration. However, this
approach caused a brief “flicker” on the initial render as the sidebar
state updated when switching say from closed to open.
...
what was rendered during hydration. The solution is in the layout file
