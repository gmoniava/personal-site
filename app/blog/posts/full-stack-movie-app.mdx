...
A hydration mismatch error happens when the HTML produced on the server doesn’t
match what React renders (in memory) on the client during hydration.

You can get such error if your rendering process is based on information that is
only available on the client side, such as local storage. But there can
also be other reasons. During SSR such client information is not available,
...
it could cause hydration mismatch error: during SSR, `sidebarOpen` would be `false`, and
