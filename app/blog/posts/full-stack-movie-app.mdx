...
### Fixing Sidebar Flicker on First Render in React

I am storing the sidebar state in local storage so that the app can remember whether it’s open or
closed. If I tried reading this value directly in the `useState` initializer function, it could
cause a hydration error: on the server, local storage is not available, so React would fall back
to a default value (like `false`). On the client, the initializer would read the actual value from
local storage (like `true`), causing a mismatch between the server-rendered DOM and what the
client expects during hydration. Besides I wasn’t sure if reading local storage directly in the
`useState` initializer function would be considered pure, so I avoided it.

Initially I had fixed this by reading the sidebar value from local storage in `useEffect` instead,
it avoided the hydration error because `useEffect` runs after hydration, so there was no difference
between the server-rendered DOM and what client expected on hydration,
but it caused a brief “flicker” on the initial render as the sidebar state updated when switching say from
closed to open.

Finally, I ended up with a solution, which had neither the flicker problem nor hydration error.
I added an inline script that runs before React hydrates, immediately applying the correct sidebar state to
the DOM - now the user doesn’t see the flicker. And also there is no hydration error because
I made sure there was no difference between the server-rendered DOM (which was modified later by the
inline script) and what client expected during hydration by making sure the side bar value
I read during the inline script and `useState` initializer were the same.
The solution is in the [`(main)/layout.tsx`](<https://github.com/gmoniava/movie-app/blob/main/src/app/(main)/layout.tsx>) file and is based on this [post](https://ethanniser.dev/blog/a-clock-that-doesnt-snap).
