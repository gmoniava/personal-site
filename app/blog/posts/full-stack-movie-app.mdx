...
match what React renders on the client during hydration.
...
Consider below approach of reading this value from local storage directly in the
`useState` initializer function in my layout file:
...
If I used this approach, it could cause hydration mismatch error: during SSR, `sidebarOpen` would be `false`, and
...
in `useState` initializer function is advised since this function has to be pure based on the docs.
