...
cause a hydration error: on the server, `localStorage` is not available, so React would fall back
...
Initially I had fixed this by reading the sidebar value from `localStorage` in `useEffect` instead,
...
but it caused a brief “flicker” on the initial render as the sidebar state updated when switching from
