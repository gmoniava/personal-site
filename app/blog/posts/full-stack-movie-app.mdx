...
- React uses the same component tree (that was used during SSR) on the client during hydration and renders it in memory,
  and then checks that this output matches the HTML sent from the server.
- React will attach event listeners to the HTML to make it interactive.
...
If I were to read this value from local storage directly in the
`useState` initializer function in my layout file as shown here:
...
it could cause hydration mismatch error: during SSR, `showSidebar` would be `false`, and
on the first render during hydration it could be `true`, which would result in
different render output. Besides I am not sure if reading local storage
in `useState` initializer function is considered pure and the initializer has to be pure based on the docs.

Also I would not be able to fix this using fix from the previous section which skips SSR for the component,
